...
def main():
    ...
    if torch.cuda.is_available():
        device = torch.device(f"cuda:{args.local_rank}") #torch.device(f"cuda:{args.gpu}" if args.gpu is not None else "cuda")
    ...
    if args.dummy:
        ...
        val_dataset = datasets.FakeData(5000, (3, 224, 224), 1000, transforms.ToTensor())
    else:
